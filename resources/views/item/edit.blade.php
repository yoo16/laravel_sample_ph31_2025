    ...
    <main class="container mx-auto px-4">
        ...
        <form enctype="multipart/form-data" action="{{ route('item.update', $item->id) }}" method="post" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="note" rows="4" class="w-full border border-gray-300 rounded-md p-2">{{ $item->note ?? old('note') }}</textarea>
            </div>
        ...
        <form method="post" action="{{ route('item.destroy', $item->id) }}" class="">
            ...
            <div class="pt-6 w-1/2">
                <button onclick="return confirm('削除しますか？')"
                    class="w-full bg-red-500 hover:bg-red-600 text-white rounded-md p-2 text-center">
                    削除
                </button>
            </div>
        </form>
